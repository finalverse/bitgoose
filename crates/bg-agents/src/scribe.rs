...
fn schema(n_sources: usize) -> serde_json::Value {
    sch::object(
        vec![
            ("working_title", sch::string_hinted("plain description of the event", "headline")),
            (
                "claims",
                sch::array(
                    sch::object(
                        vec![
                            ("text", sch::string_hinted("one self-contained sentence", "claim")),
                            ("kind", sch::enumeration(&["fact", "figure", "quote", "forecast"], "kind")),
                            ("source_indices", sch::array_n(sch::integer_index("source index"), "supporting sources", n_sources.min(3))),
                            ("excerpt", sch::string_hinted("<=20 words from a source, or empty", "excerpt")),
                            ("numeric_value", sch::string_hinted("figures only, else empty", "")),
                            ("unit", sch::string_hinted("USD, BTC, %, else empty", "")),
                        ],
                        &["text", "kind", "source_indices", "excerpt", "numeric_value", "unit"],
                    ),
                    "3-8 claims",
                ),
            ),
            ("body_md", sch::string_hinted("markdown body with [^cN] citations", "body_md")),
        ],
...
pub async fn run(ctx: &Ctx, story: StoryId) -> Result<(Vec<ClaimId>, String)> {
    ...
    stage(ctx, AgentRole::Scribe, Some(story), "draft", |run| async move {
        ...
        for dc in &draft.claims {
            ...
            for idx in &dc.source_indices {
                let Some(item) = items.get(*idx as usize) else { continue };
        ...
        bg_db::stories::set_meta(&ctx.db, story, Some(draft.working_title.trim()), None, &[], None)
            .await?;

        let note = format!("{} claims, {} words", ids.len(), bg_core::text::word_count(&draft.body_md));
