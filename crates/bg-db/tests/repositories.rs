...
async fn the_whole_graph_round_trips() {
    ...
    assert!(db.pgvector_version().await.unwrap().is_some(), "pgvector must be installed");
    assert!(!db.server_version().await.unwrap().is_empty());
    assert_eq!(db.counts().await.unwrap().len(), 9);

    // -- sources ------------------------------------------------------------
    let src = sources::upsert(
        &db, "decrypt", "Decrypt", SourceKind::Rss, "https://decrypt.co/feed",
        "https://decrypt.co", 78, 300,
    )
    .await
    .unwrap();
    let src2 = sources::upsert(
        &db, "theblock", "The Block", SourceKind::Rss, "https://theblock.co/rss.xml",
        "https://theblock.co", 82, 300,
    )
    .await
    .unwrap();
    ...
    sources::record_success(&db, src.id, Some("W/\"abc\""), Some("Mon, 01 Jan 2026 00:00:00 GMT"))
        .await
        .unwrap();
    assert_eq!(
    ...
    sources::record_failure(&db, src2.id, "connection reset").await.unwrap();
    ...
    let i1 = items::insert_new(&db, &mk(src.id, "https://a.example/1", "Exchange freezes funds"))
        .await
        .unwrap()
        .expect("first insert returns an id");
    let i2 = items::insert_new(&db, &mk(src2.id, "https://b.example/1", "Funds frozen at exchange"))
        .await
        .unwrap()
        .unwrap();

    let dupe = items::insert_new(&db, &mk(src.id, "https://a.example/1", "Exchange freezes funds"))
        .await
        .unwrap();
    assert!(dupe.is_none(), "a repeat url_hash must be silently skipped, not duplicated");
    assert_eq!(items::count(&db).await.unwrap(), 2);

    assert_eq!(items::untriaged(&db, 10).await.unwrap().len(), 2);
    items::mark_triaged(&db, i1, Some("security"), &["BTC".to_string()], 71).await.unwrap();
    items::mark_triaged(&db, i2, Some("security"), &["BTC".to_string()], 68).await.unwrap();
    ...
    let story = stories::create(
        &db, "exchange-freezes-funds", StoryKind::Desk, "Exchange freezes funds", Category::Security,
    )
    .await
    .unwrap();
    ...
    let other = stories::create(
        &db, "exchange-freezes-funds", StoryKind::Wire, "Same slug, other event", Category::Markets,
    )
    .await
    .unwrap();
    assert_ne!(other.slug, story.slug, "colliding slugs must be disambiguated");

    items::attach_to_story(&db, i1, story.id, ItemRole::Seed).await.unwrap();
    items::attach_to_story(&db, i2, story.id, ItemRole::Corroborating).await.unwrap();

    let reloaded = stories::by_id(&db, story.id).await.unwrap();
    assert_eq!(reloaded.source_count, 2, "source_count must be maintained on attach");
    assert_eq!(stories::by_slug(&db, &story.slug).await.unwrap().id, story.id);
    assert_eq!(items::by_story(&db, story.id).await.unwrap().len(), 2);
    assert_eq!(stories::source_refs(&db, story.id).await.unwrap().len(), 2);
    assert_eq!(items::clustering_candidates(&db, 24, 50).await.unwrap().len(), 2);

    stories::set_scores(&db, story.id, 77, 2.5).await.unwrap();
    stories::set_summary(&db, story.id, "An exchange froze attacker funds.").await.unwrap();
    stories::set_meta(&db, story.id, None, Some("BTC"), &["BTC".to_string()], None).await.unwrap();
    stories::set_kind(&db, story.id, StoryKind::Desk).await.unwrap();
    assert!(stories::open(&db, 10).await.unwrap().iter().any(|s| s.id == story.id));
    ...
    claims::add_source(&db, c1, i1, Stance::Supports, Some("froze the funds")).await.unwrap();
    claims::add_source(&db, c1, i2, Stance::Supports, None).await.unwrap();
    claims::add_source(&db, c2, i1, Stance::Contradicts, None).await.unwrap();

    // An over-long excerpt must be truncated by the repository before it can
    // reach the CHECK constraint.
    let long = (1..=60).map(|n| format!("w{n}")).collect::<Vec<_>>().join(" ");
    claims::add_source(&db, c2, i2, Stance::Supports, Some(&long))
        .await
        .expect("repository must truncate rather than let the DB reject it");

    claims::set_verification(&db, c1, Verification::Corroborated, 0.91).await.unwrap();
    claims::set_verification(&db, c2, Verification::SingleSource, 0.55).await.unwrap();
    ...
    assert_eq!(c2_n, 1, "the contradicting source must not count as support");
    ...
    .await
    .unwrap();
    assert_eq!(v2.version, 2, "versions must increment without a read-modify-write race");
    assert_eq!(articles::latest_for_story(&db, story.id).await.unwrap().unwrap().version, 2);
    assert_eq!(articles::by_id(&db, art.id).await.unwrap().id, art.id);

    articles::add_citations(&db, v2.id, &[("c1".to_string(), c1)]).await.unwrap();
    assert_eq!(articles::citations(&db, v2.id).await.unwrap().len(), 1);

    articles::add_correction(&db, v2.id, 1, 2, "Restated the loss figure.", "- old\n+ new", None)
        .await
        .unwrap();
    assert_eq!(articles::corrections_for_story(&db, story.id).await.unwrap().len(), 1);

    // -- publish ------------------------------------------------------------
    stories::set_status(&db, story.id, StoryStatus::Published, Some("cleared")).await.unwrap();
    ...
    assert!(published.published_at.is_some(), "publishing must stamp published_at");

    assert_eq!(stories::published(&db, Some(StoryKind::Desk), 10, 0).await.unwrap().len(), 1);
    assert_eq!(stories::published(&db, None, 10, 0).await.unwrap().len(), 1);
    assert_eq!(stories::by_category(&db, Category::Security, 10).await.unwrap().len(), 1);
    assert_eq!(stories::by_asset(&db, "btc", 10).await.unwrap().len(), 1, "asset lookup is case-insensitive");
    ...
    assert!(!wire[0].source_name.is_empty(), "wire entries must carry their lead source");
    assert!(wire[0].source_url.starts_with("http"), "wire entries must link out");

    // Unpublishing must clear the timestamp, or the CHECK constraint trips.
    stories::set_status(&db, other.id, StoryStatus::Killed, Some("duplicate")).await.unwrap();
    assert!(stories::by_id(&db, other.id).await.unwrap().published_at.is_none());
    ...
    assert_eq!(items::bodies_for_story(&db, story.id).await.unwrap().len(), 2);
    ...
    for role in AgentRole::ALL {
        agents::upsert(&db, *role, role.display_name(), "sys", 0.2).await.unwrap();
    }
    ...
    assert_eq!(stats.len(), 10, "every agent appears even with no runs today");
    ...
    assert_eq!(agents::runs_for_story(&db, story.id).await.unwrap().len(), 1);
    assert_eq!(agents::cost_since(&db, 60).await.unwrap(), Decimal::from_str("0.0125").unwrap());
    ...
    // -- entities -----------------------------------------------------------
    let e = entities::upsert(&db, EntityKind::Token, "Bitcoin", "bitcoin", Some("btc"), &["XBT".into()])
        .await
        .unwrap();
    assert_eq!(e.ticker.as_deref(), Some("BTC"), "tickers are normalized upper-case");
    let e2 = entities::upsert(&db, EntityKind::Token, "Bitcoin", "bitcoin", None, &["₿".into()])
        .await
        .unwrap();
    assert_eq!(e.id, e2.id);
    assert_eq!(e2.aliases.len(), 2, "aliases must union, not overwrite");
    assert_eq!(e2.ticker.as_deref(), Some("BTC"), "a NULL ticker must not erase a known one");
    entities::link_story(&db, e.id, story.id, 0.9).await.unwrap();
    ...
    // -- prices -------------------------------------------------------------
    prices::upsert_asset(&db, "btc", "Bitcoin", Some("bitcoin"), Some(1)).await.unwrap();
    prices::upsert_asset(&db, "ETH", "Ethereum", Some("ethereum"), Some(2)).await.unwrap();
    let now = chrono::Utc::now();
    for (sym, px, cap) in [("BTC", "62994.62", "1200000000000"), ("ETH", "3300.10", "400000000000")] {
        prices::insert_tick(
            &db,
            &PriceTick {
                symbol: sym.into(),
    ...
    assert_eq!(latest[0].symbol, "BTC", "ticker strip leads with the largest cap");
    assert!(prices::latest(&db, "btc").await.unwrap().is_some(), "symbol lookup is case-insensitive");
    ...
    assert_eq!(violations::record(&db, &report, Some(story.id), Some(v2.id), None).await.unwrap(), 1);
    assert_eq!(violations::count_blocks_24h(&db).await.unwrap(), 1);
    assert_eq!(violations::recent(&db, 10).await.unwrap().len(), 1);
    assert_eq!(violations::tally(&db, 7).await.unwrap()[0], ("quote_too_long".into(), 1));
...
async fn an_unknown_enum_token_fails_loudly_instead_of_defaulting() {
    ...
    let src = sources::upsert(
        &db, "x", "X", SourceKind::Rss, "https://x.test/feed", "https://x.test", 50, 300,
    )
    .await
    .unwrap();
    let story = stories::create(&db, "s", StoryKind::Wire, "T", Category::Markets).await.unwrap();
